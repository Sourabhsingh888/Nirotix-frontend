...
// Login Method
// Add new function
export const postJwtLogin = (data: any, headers: any = {}) => {
  return api.create(url.POST_LOGIN, data, headers);
};
...
export const getProducts = (
  ...
  api.create(url.GET_PRODUCTS_CATEGORY, {
    offset,
    limit,
    searchValue,
...
// update Products
interface Category {id : number | string, name: string; status: "Active" | "Inactive" }
export const updateProduct = (category: Category) => {
return api.put(`${url.UPDATE_PRODUCT_CATEGORY}/${category.id}`, category);
}
...
export const addProductList = (formData: FormData) =>
  api.create(url.ADD_NEW_PRODUCT, formData);


// update Product
export const updateProductList = (id: string | number, data: { price: string; currency: string }) =>
  api.put(`${url.UPDATE_PRODUCT}/${id}`, data);
...
// get Product list 
export const getProductPricingList = (
offset: number = 0,
limit: number = 10,
searchValue: string = "",
  
) => api.create(url.GET_PRODUCT_PRICING, {
offset,
limit, 
searchValue,
});
...
// ✅ Update Product Pricing API Helper
export const updateProductPricingList = (productdata: {
  id: string | number;
  price: string | number;
  currency: string;
}) => api.put(`${url.UPDATE_PRODUCT_PRICING}/${productdata.id}`, {
